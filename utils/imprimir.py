def imprimir(database: list, i: int):
    ...
    if type(exten) == float:#verifica se a variavel é do tipo float
      print(f"{database[i]['nome']}\t extensão: {database[i]['extensão']:.3f} KM\t ano de criação: {database[i]['ano']}")
    else:
           print(f"{database[i]['nome']}\t extensão: {database[i]['extensão']:.>5} KM\t ano de criação: {database[i]['ano']}")
